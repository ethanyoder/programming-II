...
public class PatternMatcher extends JFrame{
	...
	public int getMatches(){
		...
		//creates objects to set up a regular expression - text match method
		Pattern p = Pattern.compile(patternText);
		Matcher m = p.matcher(inputText);
		//creates a counter variable that will increment whenever the find method returns true
		int counter = 0;
		while (m.find())
			counter ++;
		return counter; // don't forget to change this
	}
	...
	public int getNumOfWords(){
		...
		/* this object has each word become a token (separated by spaces).  The countTokens method
		   then used to figure out the number of words in the text.
		*/
		StringTokenizer stk = new StringTokenizer(inputText);
